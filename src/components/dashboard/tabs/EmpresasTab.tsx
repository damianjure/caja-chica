              ...
              {companySummaries.map((company) => (
                <div key={company.name} className="rounded-2xl border border-neutral-200 p-5 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm md:grid-cols-3">
                    <div>
                      <div className="text-neutral-400 uppercase tracking-widest text-[10px] mb-1">Ingresos ARS</div>
                      <div className="font-medium text-green-600">{formatCurrency(company.ingresosArs, 'ARS')}</div>
                    </div>
                    <div>
                      <div className="text-neutral-400 uppercase tracking-widest text-[10px] mb-1">Gastos ARS</div>
                      <div className="font-medium text-red-600">{formatCurrency(company.gastosArs, 'ARS')}</div>
                    </div>
                    <div>
                      <div className="text-neutral-400 uppercase tracking-widest text-[10px] mb-1">Saldo ARS</div>
                      <div className={`font-medium ${company.saldoArs >= 0 ? 'text-green-600' : 'text-red-600'}`}>{formatCurrency(company.saldoArs, 'ARS')}</div>
                    </div>
                    <div>
                      <div className="text-neutral-400 uppercase tracking-widest text-[10px] mb-1">Ingresos USD</div>
                      <div className="font-medium text-green-600">{formatCurrency(company.ingresosUsd, 'USD')}</div>
                    </div>
                    <div>
                      <div className="text-neutral-400 uppercase tracking-widest text-[10px] mb-1">Gastos USD</div>
                      <div className="font-medium text-red-600">{formatCurrency(company.gastosUsd, 'USD')}</div>
                    </div>
                    <div>
                      <div className="text-neutral-400 uppercase tracking-widest text-[10px] mb-1">Saldo USD</div>
