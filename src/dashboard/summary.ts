...
export function getCategorySummaries(history: Movimiento[], companyName?: string) {
  ...
  // Optimization: Single for...of loop avoids intermediate array allocations
  // from chaining .filter().filter().forEach() on a large history dataset.
...
export function getIncomeSummaries(history: Movimiento[]) {
  ...
  // Optimization: Single for...of loop avoids intermediate array allocations
  // from chaining .filter().forEach() on a large history dataset.
  for (const item of history) {
