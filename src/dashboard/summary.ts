...
export function getCategorySummaries(history: Movimiento[], companyName?: string) {
  ...
  // Replaced chained .filter().filter().forEach() with a single for...of loop.
  // Impact: Prevents multiple intermediate array allocations and reduces iteration overhead
  // when processing large datasets arrays like history.
...
export function getIncomeSummaries(history: Movimiento[]) {
  ...
  // ⚡ Bolt Performance Optimization:
  // Replaced chained .filter().forEach() with a single for...of loop.
  // Impact: Prevents intermediate array allocation and reduces iteration overhead.
  for (const item of history) {
