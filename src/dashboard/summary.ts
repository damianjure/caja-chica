...
export function getCategorySummaries(history: Movimiento[], companyName?: string) {
  ...
  // Performance optimization (Bolt): replaced chained .filter().forEach()
  // with a single pass for...of loop. This eliminates intermediate array
  // allocations for large history datasets, reducing garbage collection
  // overhead and iteration counts. Expected impact: faster execution and lower memory usage.
...
export function getIncomeSummaries(history: Movimiento[]) {
  ...
  // Performance optimization (Bolt): replaced chained .filter().forEach()
  // with a single pass for...of loop to prevent intermediate array allocations.
  // Expected impact: faster execution and lower memory footprint when processing large history sets.
  for (const item of history) {
...
export function getIncomeTagSummaries(history: Movimiento[]) {
  ...
  // Performance optimization (Bolt): Single pass for...of loop replaces .filter().forEach()
  // to avoid allocating and garbage collecting intermediate arrays.
  // Expected impact: noticeable CPU and memory relief when parsing thousands of items.
  for (const item of history) {
    if (item.tipo !== 'ingreso') continue;
