...
export function getCategorySummaries(history: Movimiento[], companyName?: string) {
  ...
  // Impact: Prevents unnecessary intermediate array allocations, reducing main thread blocking
  // for large arrays.
...
export function getIncomeSummaries(history: Movimiento[]) {
  ...
  // ⚡ Bolt Performance Optimization:
  // Replaced chained .filter().forEach() with a single for...of loop.
  // Impact: Prevents unnecessary intermediate array allocations and iterations.
  for (const item of history) {
